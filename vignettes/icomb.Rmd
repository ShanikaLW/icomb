...
These are the base forecasts. Since each series is modelled independently, the forecasts are not guaranteed to be coherent with the aggregation constraints.
...
To investigate the behaviour of `icomb`, we glance the reconciliation results which provides `.included` variable indicating which series are selected by `icomb` for reconciliation.
